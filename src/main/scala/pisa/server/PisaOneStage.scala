  ...
  def deal_with_apply_to_tls(toplevel_state_name: String, action: String, new_name: String, timeout_in_millis: Int): String = {
    if (pisaos.top_level_state_map.contains(toplevel_state_name)) {
      ...
      if (action == GAP_STEP) {
        ...
        try {
          val total_result = pisaos.exp_with_hammer(old_state, timeout_in_millis=32000)
      ...
      println("will call register_tls...: " + new_name)
...
//        }
          ...
          try {
            deal_with_apply_to_tls(tls_name, action, new_name, timeout_in_millis = timeout * 1000)
          } catch {
